...
import { ImportSeekersForm } from "./ImportSeekersForm";
...
export default async function ImportPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-xl font-bold text-[var(--ink)] mb-2">Import Job Seekers</h1>
      <p className="text-sm text-[var(--ink-muted)] mb-6">
        Upload a CSV file (from Excel: <strong>File → Save As → CSV UTF-8</strong>) with column headers. Only{" "}
        <strong>Full Name</strong> and <strong>Phone Number</strong> are required — a row is skipped (and
        ...
        <strong>
          Highest Qualification, Work Experience, Desired Minimum Salary, Specialization / Skills, Previous Job
          Details, Any Other Information, Address, Gender, Age
        </strong>
        . Every row in one file is assigned the same District and Approver, picked below — for a mixed-district
        list, split it into separate files first.
      </p>
      ...
      >
        <ImportSeekersForm districts={districts} approvers={activeApprovers} />
      </div>
    </div>
  );
}
